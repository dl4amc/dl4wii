...
Run code for CNN with the 4 MHz dataset for the following experiments:
...
dataset_path = '../data'
# Dataset Types - 'fft iq 4mhz', 'fft amp phase 4mhz'
dataset_type = 'fft amp phase 4mhz'
# Experiment Types - 'pcs', 'autoencoder', 'pca', 'high magnitude', 'low magnitude', 'uniform', 'random'
experiment_type = 'pcs'
# Final Dimensions - 26 (1/2), 13 (1/4), 7 (1/8)
final_dimensions = 26
...
def load_data(storage_folder, which_kind):
    ...
    if which_kind == 'fft iq 4mhz':
        data_file = 'data_fft_subset.p'
    elif which_kind == 'fft amp phase 4mhz':
        data_file = 'data_fft_amplitude_phase_subset.p'
    else:
        raise ValueError('Parameter which_kind must be a valid dataset')
    # load input data (x)
...
class CNN(nn.Module):
    def __init__(self):
        super().__init__()
        if final_dimensions == 26:
            self.arch_dims = 22
        elif final_dimensions == 13:
            self.arch_dims = 9
        elif final_dimensions == 7:
            self.arch_dims = 3
...
def run_experiment(exp_type, exp_dims, x_train, x_test):
    ...
    if exp_type == 'pcs':
        ...
        x_train_new = x_train_new.transpose((2, 0, 1))
        x_train_new = np.array(np.split(x_train_new, 210))
        x_train_new = np.array(np.split(x_train_new, 10))
        ...
        x_test_new = x_test_new.transpose((2, 0, 1))
        x_test_new = np.array(np.split(x_test_new, 210))
        x_test_new = np.array(np.split(x_test_new, 10))
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        input_dim = Input(shape = (104, ))
        encoding_dim = 2 * ae_rate
        # Encoder
        encoded1 = Dense(78, activation = 'relu')(input_dim)
        encoded2 = Dense(52, activation = 'relu')(encoded1)             # Output if final_dims = 26
        encoded3 = Dense(26, activation = 'relu')(encoded2)             # Output if final_dims = 13
        encoded4 = Dense(encoding_dim, activation = 'relu')(encoded3)   # Output if final_dims = 7
        # Decoder
        decoded1 = Dense(26, activation = 'relu')(encoded4)
        decoded2 = Dense(52, activation = 'relu')(decoded1)
        decoded3 = Dense(78, activation = 'relu')(decoded2)
        decoded4 = Dense(104, activation = 'sigmoid')(decoded3)
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        n_samples = exp_dims
        sample_idx = [num for num in range(0, 52, 52//n_samples)]
        x_train_new = np.concatenate(x_train, axis=0)
        x_train_new = np.concatenate(x_train_new, axis=0)
        x_train_new = x_train_new.transpose((1, 0, 2))
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        # Random Sub Sampling
        n_samples = exp_dims
        sample_idx = np.random.choice(range(0,52), size=n_samples, replace=False)
        x_train_new = np.concatenate(x_train, axis=0)
        x_train_new = np.concatenate(x_train_new, axis=0)
        x_train_new = x_train_new.transpose((1, 0, 2))
        x_train_new = x_train_new[sample_idx]
...
#########################################################################################
# for training
x_train_training = x_train.contiguous().view(10*21*480, 1, 26, 2)
y_train_training = y_train.contiguous().view(10*21*480)
x_test_training = x_test.contiguous().view(10*21*235, 1, 26, 2)
y_test_training = y_test.contiguous().view(10*21*235)

# for plot
x_test_plot = x_test.contiguous().view(10, 21, 235, 1, 26, 2)
y_test_plot = y_test
...
with torch.no_grad():
    for i in range(10):
        ...
        input = x_test_plot[i].cuda().contiguous().view(21*235, 1, 26, 2)
